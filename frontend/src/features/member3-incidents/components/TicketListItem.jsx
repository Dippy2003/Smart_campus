...
export default function TicketListItem({ ticket, onOpen, footer }) {
  const unreadCount = (ticket.notifications || []).filter((n) => !n.read).length;

  return (
    <div className="w-full overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/60 shadow-md shadow-slate-950/60 transition hover:-translate-y-0.5 hover:border-blue-500/60 hover:shadow-blue-900/40">
      <button
        type="button"
        onClick={onOpen}
