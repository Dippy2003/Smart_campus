    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http, OAuth2UserProvisioner provisioner) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.POST, "/resource").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.PUT, "/resource/**").hasRole("ADMIN")
                        ...
                        // Incidents / Tickets (Member 3) — staff ticket console is TECHNICIAN-only; users keep /my + notification read
                        .requestMatchers(HttpMethod.POST, "/api/incidents").authenticated()
                        .requestMatchers(HttpMethod.GET, "/api/incidents/my").authenticated()
                        .requestMatchers(HttpMethod.GET, "/api/incidents/technician").hasRole("TECHNICIAN")
                        .requestMatchers(HttpMethod.GET, "/api/incidents/technicians").hasRole("TECHNICIAN")
                        .requestMatchers(HttpMethod.GET, "/api/incidents").hasRole("TECHNICIAN")
                        .requestMatchers(HttpMethod.GET, "/api/incidents/*").hasRole("TECHNICIAN")
                        .requestMatchers(HttpMethod.PUT, "/api/incidents/*/notifications/read").authenticated()
                        .requestMatchers(HttpMethod.PUT, "/api/incidents/*/status").hasRole("TECHNICIAN")
                        .requestMatchers(HttpMethod.POST, "/api/incidents/*/reply").hasRole("TECHNICIAN")
